 ...
 * ============================================================================
 *
 * 【为什么走 API 而不是操作页面 DOM】
 * 旧版实现依赖库存页的 DOM 结构：注入复选框 → 点击 Steam 自带卖框 → 填价 → 等人工确认。
 * 这条路线有两个无法根治的问题：
 *   1. Steam 每次改版都会让选择器失效，脚本直接报废；
 *   2. 每张卡都要走一遍 UI 流程，速度被 UI 完全限制。
 * 现在改为直读 Steam 自身的 AJAX 端点，不再依赖任何 DOM 结构。
 *
 * 【库存数据为什么从 /inventory/ 端点取，而不是读页面全局对象】
 * 实测 g_ActiveInventory 只代表"当前选中游戏"的库存，且其 m_rgAssets 是按需填充的
 * （只装已渲染页），m_rgPages 里的 CPage 只含 DOM 分页信息不含物品数组。
 * 因此脚本完全不碰 g_ActiveInventory，改为一次请求拿全量：
 *   GET /inventory/{steamid}/753/6?count=2000
 * 返回 assets[]（实例级，433 条）+ descriptions[]（类型级，373 条），
 * 二者通过 classid + instanceid 关联（实测 0 条失配）。
 *
 * 【为什么 @grant none】
 * 需要读取页面的 g_rgWalletInfo（钱包货币 / 费率 / 最低价）以及调用页面的
 * GetItemPriceFromTotal 做价格反推。这些都在页面主世界，沙箱内拿不到。
 * @grant none 让脚本直接在主世界执行，代价是必须用 IIFE 严格包裹，
 * 绝不能往 window 上挂任何全局变量（会污染页面，也可能与其他脚本打架）。
 *
 * 【为什么价格用"卖家实收"而不是"买家支付"】
 * 实测 Steam 卖框源码：提交给 /market/sellitem/ 的 price 参数是
 *   m_nConfirmedPrice = GetPriceAsInt() = market_sell_currency_input 的值
 * 即【卖家实收金额（单位：分）】，不是买家支付价。
 * 传错这个值会造成真实的价格事故（实测误传 3 分导致卡以远低于市场价被秒买）。
 *
 * 【手续费规则（源码级推导，务必理解）】
 *   ToValidMarketPrice(n)   : n <= wallet_market_minimum 一律返回该下限
 *   CalculateFee(base, pct) : ToValidMarketPrice(floor(base * pct))
 *   买家支付 = 实收 + CalculateFee(实收, 发行商费率) + CalculateFee(实收, Steam费率)
 * 关键坑位：各项费用都有下限兜底，所以极低价商品的费率会异常高。
 *   例：¥0.21 的卡 → 实收 7 分 + Steam费 7 分 + 发行商费 7 分 = 买家支付 21 分
 *   手续费占 2/3。面板因此必须显示"预计到手"，否则用户会严重误判收益。
 *
 * 【手机令牌是硬约束】
 * 上架接口只能"发起"，真正生效必须在 Steam 手机 App 里确认。
 * 实测响应会带 needs_mobile_confirmation: true。
 * 好消息是手机端确认页支持批量勾选，所以正确用法是：
 *   脚本批量发起 → 一次性去手机上全部勾选确认。
 * 脚本无法、也不应该绕过这一步（那需要用户的令牌 secret）。
 * ============================================================================
 */
